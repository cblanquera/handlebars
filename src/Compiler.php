...
class Compiler
{
     ...
     *
     * @param *Handler $handlebars
     * @param *string  $source
     */
    public function __construct(Handler $handlebars, $source)
    {
        $this->source = $source;
    ...
    public function compile($layout = true)
    {
        ...
        $tokenizer->tokenize($callback);

        if (count($reference->open)) {
            throw Exception::forMissingClosing($reference->open);
        }
    ...
    {
        $callback = function ($node) use ($reference) {
            switch ($node['type']) {
    ...
    protected function generateClose(array $node, array &$open)
    {
        ...
        if ($this->findSection($open, $node['value']) === false) {
            throw Exception::forUnknownEnd($node['value'], $node['line']);
        }
